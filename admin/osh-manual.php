...
// Fetch all OSH Manual records with joins
$records = [];
$res = $db->query("
    ...
    ORDER BY om.sno DESC, om.created_at DESC
");
if ($res !== false) {
    $records = $res->fetch_all(MYSQLI_ASSOC);
} else {
    error_log('Error fetching OSH Manual records: ' . $db->error);
    $records = [];
}
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4 class="card-title">OSH Manual</h4>
                                <a href="osh-manual-form.php" class="btn btn-sm btn-outline-primary">
                                    <i class="fas fa-plus"></i> Add New Manual
                                </a>
                            </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="oshManualTable" class="display min-w850 table table-striped table-hover">
                                        <thead>
                                            <tr>
                                                <th>S/N</th>
                                                <th>Description</th>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($records as $r): ?>
                                                    <tr>
                                                        ...
                                                        <td><?= htmlspecialchars($r['manual_no']) ?></td>
                                                        <td>
                                                            <span class="badge badge-<?= 
                                                                $r['rev_status'] === 'Current' ? 'success' : 
                                                                ($r['rev_status'] === 'Revised' ? 'warning' : 'secondary')
                                                            ?>">
                                                                <?= htmlspecialchars($r['rev_status']) ?>
                                                            </span>
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <div class="d-flex">
                                                                <a href="edit-osh-manual.php?id=<?= $r['id'] ?>" class="btn btn-sm btn-outline-secondary">
                                                                    <i class="fas fa-edit"></i> Edit
                                                                </a>

                                                                <form action="action/osh-manual-delete.php" method="post" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this manual?');">
