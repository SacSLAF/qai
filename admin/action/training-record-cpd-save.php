...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    header('Location: ../training-record-cpd-form.php');
    exit();
}
...
if (!empty($missing_fields)) {
    ...
    header('Location: ../training-record-cpd-form.php?error=1');
    exit();
}
...
$has_new_file = isset($_FILES['record_file']) && $_FILES['record_file']['error'] === UPLOAD_ERR_OK;
...
if (!$is_edit_mode && !$has_new_file) {
    $_SESSION['error'] = "Record file is required for new records";
    header('Location: ../training-record-cpd-form.php?error=2');
    exit();
}
...
if ($is_edit_mode && !$has_new_file && !$has_existing_file) {
    $_SESSION['error'] = "Record file is required";
    header('Location: ../training-record-cpd-form.php?error=2');
    exit();
}
...
if ($has_new_file) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        ...
        header('Location: ../training-record-cpd-form.php?error=4');
        exit();
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024) {
        ...
        header('Location: ../training-record-cpd-form.php?error=5');
        exit();
    }

    // Create upload directory
    $upload_dir = 'uploads/training_record_cpd/';
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            $_SESSION['error'] = "Failed to create upload directory: $upload_dir";
            header('Location: ../training-record-cpd-form.php?error=3');
            exit();
        }
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $temp_path)) {
        ...
        header('Location: ../training-record-cpd-form.php?error=6');
        exit();
    }
...
try {
    ...
    if ($is_edit_mode) {
        ...
        if ($file_uploaded) {
            ...
            $final_file_path = 'uploads/training_record_cpd/' . basename($temp_path);
        } else {
            // Keep existing file
            $final_file_path = $existing_file_path;
        }
        
        $sql = "UPDATE training_record_cpd SET 
                sno = ?, trade = ?, description = ?, duration = ?, 
                ac_categories_id = ?, file_path = ?, created_by = ?, updated_at = NOW()
                WHERE id = ?";
        ...
        $stmt->bind_param(
            'ssssisii',
            $sno, $trade, $description, $duration,
            $ac_categories_id, $final_file_path, $created_by, $id
        );
        ...
        $sql = "INSERT INTO training_record_cpd (
            sno, trade, description, duration, ac_categories_id, file_path, created_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param(
            'ssssisi',
            $sno, $trade, $description, $duration,
            $ac_categories_id, $final_file_path, $created_by
        );
    ...
    // FILE RENAMING LOGIC
    if ($file_uploaded) {
        $new_name = 'cpd_' . $last_id . '.' . $file_extension;
        $new_path = $upload_dir . $new_name;
        $new_file_path = 'uploads/training_record_cpd/' . $new_name;
        ...
        $update_stmt = $db->prepare("UPDATE training_record_cpd SET file_path = ? WHERE id = ?");
        if (!$update_stmt) {
    ...
    $_SESSION['success'] = "Training Record CPD " . ($is_edit_mode ? 'updated' : 'saved') . " successfully!";
    ...
    header('Location: ../training-record-cpd-form.php?error=7');
    exit();
}