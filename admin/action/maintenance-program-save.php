...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    header('Location: ../maintenance-program-form.php');
    exit();
}

// Debug: Log POST data
error_log("Maintenance Program Save - POST Data: " . print_r($_POST, true));
error_log("Maintenance Program Save - FILES Data: " . print_r($_FILES, true));
...
foreach ($required_fields as $field => $value) {
    if (empty($value)) {
        ...
        header('Location: ../maintenance-program-form.php?error=1');
        exit();
    }
}
...
$has_new_file = isset($_FILES['document_file']) && $_FILES['document_file']['error'] === UPLOAD_ERR_OK;

if (!$is_edit_mode && !$has_new_file) {
    $_SESSION['error'] = "Document file is required for new records";
    header('Location: ../maintenance-program-form.php?error=2');
    exit();
}
...
if ($has_new_file) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        ...
        header('Location: ../maintenance-program-form.php?error=4');
        exit();
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024) {
        ...
        header('Location: ../maintenance-program-form.php?error=5');
        exit();
    }
    ...
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            ...
            header('Location: ../maintenance-program-form.php?error=3');
            exit();
        }
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $temp_path)) {
        ...
        header('Location: ../maintenance-program-form.php?error=6');
        exit();
    }
...
try {
    ...
    if ($is_edit_mode) {
        ...
        if ($file_uploaded) {
            ...
            $final_file_path = 'uploads/maintenance/' . basename($temp_path);
        } else {
            // Keep existing file
            $final_file_path = $existing_file_path;
        }
        
        $sql = "UPDATE maintenance_documents SET 
                document_type = ?, document_number = ?, description = ?, formation_id = ?, 
                trade = ?, type_id = ?, issue = ?, revision = ?, revision_date = ?, 
                branch_id = ?, file_path = ?
                WHERE id = ?";
        ...
        $stmt->bind_param(
            'sssisisssssi',
            $document_type, $document_number, $description, $formation_id,
            $trade, $type_id, $issue, $revision, $revision_date_db,
            $branch_id, $final_file_path, $id
        );
        ...
        $sql = "INSERT INTO maintenance_documents (
            document_type, document_number, description, formation_id, trade, 
            type_id, issue, revision, revision_date, branch_id, file_path
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    // FILE RENAMING LOGIC
    if ($file_uploaded) {
        $new_name = 'doc_' . $last_id . '.' . $file_extension;
        $new_path = $upload_dir . $new_name;
        $new_file_path = 'uploads/maintenance/' . $new_name;
        ...
        $update_stmt = $db->prepare("UPDATE maintenance_documents SET file_path = ? WHERE id = ?");
        if (!$update_stmt) {
    ...
    $_SESSION['success'] = "Maintenance document saved successfully!";
    ...
    header('Location: ../maintenance-program-form.php?error=7');
    exit();
}
?>