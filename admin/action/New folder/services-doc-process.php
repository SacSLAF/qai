    ...
    try {
        ...
        if ($service_category_id == 2) { // Aircraft Competency
            if (isset($_POST['svc_no']) && isset($_POST['rank']) && isset($_POST['name']) && 
                isset($_POST['aircraft_type']) && isset($_POST['last_competency']) && 
                isset($_POST['renewal_date']) && isset($_POST['currency']) && isset($_POST['squadron'])) {
                
                $stmt = $db->prepare("INSERT INTO aircraft_competency (title, description, svc_no, rank, name, aircraft_type, last_level_of_competency, renewal_date, currency, squadron, uploaded_by, file_path, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                ...
                $stmt->bind_param("ssssssssssisi", $title, $description, $_POST['svc_no'], $_POST['rank'], $_POST['name'], 
                                 $_POST['aircraft_type'], $_POST['last_competency'], $_POST['renewal_date'], 
                                 $_POST['currency'], $_POST['squadron'], $uploaded_by, $new_path, $is_active);
