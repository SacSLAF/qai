...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    header('Location: ../outside-training-form.php');
    exit();
}
...
$conducted_institute = isset($_POST['conducted_institute']) ? trim($_POST['conducted_institute']) : null;
...
if (!empty($missing_fields)) {
    ...
    header('Location: ../outside-training-form.php?error=1');
    exit();
}
...
$has_new_file = isset($_FILES['training_file']) && $_FILES['training_file']['error'] === UPLOAD_ERR_OK;
...
if (!$is_edit_mode && !$has_new_file) {
    $_SESSION['error'] = "Training file is required for new records";
    header('Location: ../outside-training-form.php?error=2');
    exit();
}
...
if ($is_edit_mode && !$has_new_file && !$has_existing_file) {
    $_SESSION['error'] = "Training file is required";
    header('Location: ../outside-training-form.php?error=2');
    exit();
}
...
if ($has_new_file) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        ...
        header('Location: ../outside-training-form.php?error=4');
        exit();
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024) {
        ...
        header('Location: ../outside-training-form.php?error=5');
        exit();
    }

    // Create upload directory
    $upload_dir = 'uploads/outside_training/';
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            $_SESSION['error'] = "Failed to create upload directory: $upload_dir";
            header('Location: ../outside-training-form.php?error=3');
            exit();
        }
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $temp_path)) {
        ...
        header('Location: ../outside-training-form.php?error=6');
        exit();
    }
...
try {
    ...
    if ($is_edit_mode) {
        ...
        if ($file_uploaded) {
            ...
            $final_file_path = 'uploads/outside_training/' . basename($temp_path);
        } else {
            // Keep existing file
            $final_file_path = $existing_file_path;
        }
        
        $sql = "UPDATE outside_training SET 
                sno = ?, description = ?, conducted_institute = ?, duration = ?, 
                file_path = ?, created_by = ?, updated_at = NOW()
                WHERE id = ?";
        ...
        $sql = "INSERT INTO outside_training (
            sno, description, conducted_institute, duration, file_path, created_by
        ) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    // FILE RENAMING LOGIC
    if ($file_uploaded) {
        $new_name = 'outside_training_' . $last_id . '.' . $file_extension;
        $new_path = $upload_dir . $new_name;
        $new_file_path = 'uploads/outside_training/' . $new_name;
        ...
        $update_stmt = $db->prepare("UPDATE outside_training SET file_path = ? WHERE id = ?");
        if (!$update_stmt) {
    ...
    $_SESSION['success'] = "Outside Training " . ($is_edit_mode ? 'updated' : 'saved') . " successfully!";
    ...
    header('Location: ../outside-training-form.php?error=7');
    exit();
}