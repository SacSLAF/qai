...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    header('Location: ../audit-report-form.php');
    exit();
}
...
$conducted_date = isset($_POST['conducted_date']) ? $_POST['conducted_date'] : '';
$productivity_category_id = isset($_POST['productivity_category_id']) ? (int)$_POST['productivity_category_id'] : 0;
...
$required_fields = [
    ...
    'Conducted Date' => $conducted_date,
    'Productivity Category' => $productivity_category_id
];
...
if (!empty($missing_fields)) {
    ...
    header('Location: ../audit-report-form.php?error=1');
    exit();
}

// Validate date
if (!strtotime($conducted_date)) {
    $_SESSION['error'] = "Invalid conducted date format";
    header('Location: ../audit-report-form.php?error=1');
    exit();
}
...
$has_new_file = isset($_FILES['report_file']) && $_FILES['report_file']['error'] === UPLOAD_ERR_OK;
...
if (!$is_edit_mode && !$has_new_file) {
    $_SESSION['error'] = "Report file is required for new records";
    header('Location: ../audit-report-form.php?error=2');
    exit();
}
...
if ($is_edit_mode && !$has_new_file && !$has_existing_file) {
    $_SESSION['error'] = "Report file is required";
    header('Location: ../audit-report-form.php?error=2');
    exit();
}
...
if ($has_new_file) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        ...
        header('Location: ../audit-report-form.php?error=4');
        exit();
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024) {
        ...
        header('Location: ../audit-report-form.php?error=5');
        exit();
    }

    // Create upload directory
    $upload_dir = 'uploads/audit_reports/';
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            $_SESSION['error'] = "Failed to create upload directory: $upload_dir";
            header('Location: ../audit-report-form.php?error=3');
            exit();
        }
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $temp_path)) {
        ...
        header('Location: ../audit-report-form.php?error=6');
        exit();
    }
...
try {
    ...
    if ($is_edit_mode) {
        ...
        if ($file_uploaded) {
            ...
            $final_file_path = 'uploads/audit_reports/' . basename($temp_path);
        } else {
            // Keep existing file
            $final_file_path = $existing_file_path;
        }
        
        $sql = "UPDATE audit_report SET 
                sno = ?, slaf_establishment_id = ?, conducted_date = ?, 
                productivity_category_id = ?, section_id = ?, file_path = ?, uploaded_by = ?, updated_at = NOW()
                WHERE id = ?";
        ...
        $stmt->bind_param(
            'sisiiisi',
            $sno, $slaf_establishment_id, $conducted_date,
            $productivity_category_id, $section_id, $final_file_path, $uploaded_by, $id
        );
        ...
        $sql = "INSERT INTO audit_report (
            sno, slaf_establishment_id, conducted_date, productivity_category_id, 
            section_id, file_path, uploaded_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param(
            'sisiiis',
            $sno, $slaf_establishment_id, $conducted_date,
            $productivity_category_id, $section_id, $final_file_path, $uploaded_by
        );
    ...
    // FILE RENAMING LOGIC
    if ($file_uploaded) {
        $new_name = 'ar_' . $last_id . '.' . $file_extension;
        $new_path = $upload_dir . $new_name;
        $new_file_path = 'uploads/audit_reports/' . $new_name;
        ...
        $update_stmt = $db->prepare("UPDATE audit_report SET file_path = ? WHERE id = ?");
        if (!$update_stmt) {
    ...
    $_SESSION['success'] = "Audit report " . ($is_edit_mode ? 'updated' : 'saved') . " successfully!";
    header('Location: ../audit-report.php?success=1');
    exit();
    ...
    header('Location: ../audit-report-form.php?error=7');
    exit();
}
?>