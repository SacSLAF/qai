...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    header('Location: ../tech-library-form.php');
    exit();
}

// Collect fields
$sno = trim($_POST['sno'] ?? '');
$publication_index = trim($_POST['publication_index'] ?? '');

// Validate required fields
if (empty($sno) || empty($publication_index)) {
    $_SESSION['error'] = "All required fields must be filled";
    header('Location: ../tech-library-form.php?error=1');
    exit();
}
...
if (!$is_edit_mode && !$has_new_file) {
    ...
    header('Location: ../tech-library-form.php?error=2');
    exit();
}
...
if ($has_new_file) {
    ...
    if (!in_array($file_extension, $allowed_extensions)) {
        $_SESSION['error'] = "Only PDF, DOC, DOCX, XLS, XLSX, JPG, JPEG, PNG files are allowed. Your file: .$file_extension";
        header('Location: ../tech-library-form.php?error=4');
        exit();
    }
    ...
    if ($file['size'] > 10 * 1024 * 1024) {
        ...
        header('Location: ../tech-library-form.php?error=5');
        exit();
    }

    // Create upload directory
    $upload_dir = 'uploads/tech-library/';
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            $_SESSION['error'] = "Failed to create upload directory: $upload_dir";
            header('Location: ../tech-library-form.php?error=3');
            exit();
        }
    }
    ...
    if (!move_uploaded_file($file['tmp_name'], $temp_path)) {
        ...
        header('Location: ../tech-library-form.php?error=6');
        exit();
    }
...
try {
    ...
    if ($is_edit_mode) {
        ...
        if ($file_uploaded) {
            ...
            $final_file_path = 'uploads/tech-library/' . basename($temp_path);
        } else {
            // Keep existing file
            $final_file_path = $existing_file_path;
        }
        
        $sql = "UPDATE tech_library SET 
                sno = ?, publication_index = ?, file_path = ?
                WHERE id = ?";
        ...
        $stmt->bind_param('sssi', $sno, $publication_index, $final_file_path, $id);
        ...
        $sql = "INSERT INTO tech_library (sno, publication_index, file_path) VALUES (?, ?, ?)";
        ...
        $stmt->bind_param('sss', $sno, $publication_index, $final_file_path);
    ...
    if ($file_uploaded) {
        ...
        $new_file_path = 'uploads/tech-library/' . $new_name;
        ...
        $update_stmt = $db->prepare("UPDATE tech_library SET file_path = ? WHERE id = ?");
        if (!$update_stmt) {
    ...
    $_SESSION['success'] = "Technical library document saved successfully!";
    ...
    header('Location: ../tech-library-form.php?error=7');
    exit();
}
?>